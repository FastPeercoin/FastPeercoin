...
void CWallet::AvailableCoins(std::vector<COutput>& vCoins, unsigned int nSpendTime, bool fOnlyConfirmed) const
{
    vCoins.clear();
...
bool CWallet::SelectCoins(int64 nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    AvailableCoins(vCoins, nSpendTime, true);
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            ploop
            {
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // ppcoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
